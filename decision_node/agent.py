...
def perception_node(state: AgentState):
    ...
    # 1. UPDATED SCRUBBER: Only block people during INSPECTION
    people_words = ["person", "man", "men", "women", "woman", "t-shirt", "shirt", "walking", "pedestrian"]
    
    # If we are inspecting the bridge and a person blocks the camera, scrub it.
    ...
    # 2. Standard Garbage Scrubber (applies to all missions)
...
def cognition_node(state: AgentState):
    mission = MISSIONS[state["mission_key"]]
    raw_yolo = state['yolo_report'].replace("YOLO sees: ", "")
    
    # 1. Calculate counts for the context
    if raw_yolo != "Clear":
        labels = [obj.strip() for obj in raw_yolo.split(",")]
        total_objects = len(labels)
        counts = {name: labels.count(name) for name in set(labels)}
        # Create a summary for the LLM that includes the total
        yolo_summary = " | ".join([f"{k.capitalize()}: {v}" for k, v in counts.items()])
        yolo_summary += f" | Total: {total_objects}"
    else:
        yolo_summary = "No objects detected"
    
    context = f"COUNTS: {yolo_summary} | VLM_RAW: {state['visual_analysis']}"
    
    # 2. UPDATE PROMPT: Explicitly demand the Results Table
    if state["mission_key"] == "surveillance":
        user_prompt = (
            f"Generate a formal technical report. You MUST include these exact sections: "
            "1. Introduction, 2. Methodology, 3. Results (Include a Markdown table with 'Category', 'Count', "
            "and 'Percentage of Total'), and 4. Discussion. "
            f"Data: {context}. Start the report after the word 'REASON:'."
        )
    else:
        user_prompt = f"Provide a one-sentence structural health summary after 'REASON:'. Data: {context}"
    ...
    # This logic now works for BOTH Status: CRITICAL and Status: ACTIVE
    if "STATUS: CRITICAL" in content or "STATUS: ACTIVE" in content:
        status = "CRITICAL" if "CRITICAL" in content else "ACTIVE"
    else:
        status = "HEALTHY" if state["mission_key"] == "inspection" else "CLEAR"
    ...
    return {
        ...
        "yolo_report": yolo_summary # We reuse yolo_report to ensure it passes through the state
    }
